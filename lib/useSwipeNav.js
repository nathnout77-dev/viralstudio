...
export default function useSwipeNav({ order, current, onNavigate, enabled = true }) {
  ...
  useEffect(() => {
    ...
    const onEnd = (e) => {
      ...
      if (i === -1) return
      // Sens spatial : on glisse vers la gauche → on va sur l'onglet de GAUCHE
      // (celui affiché à gauche dans la barre du bas), et inversement. Le doigt
      // désigne la destination, il ne pousse pas le contenu.
