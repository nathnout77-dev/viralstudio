...
export function matchBibliotheque(vin) {
  ...
  if (!app) return null
  return WINE_DB.find(w => {
    const wa = normaliser(w.appellation)
    return wa === app || app.includes(wa) || wa.includes(app)
  }) || null
}
...
export function construirePrediction(vin) {
  ...
  return {
    source,
    fiabilite: {
      bibliotheque: { label: 'Fiche Œno — prédiction fiable', emoji: '📖' },
      verifie: { label: 'Données connues de ce vin — prédiction solide', emoji: '🔎' },
      cepages: { label: `Déduit des cépages (${cepages.join(', ')})`, emoji: '🍇' },
      type: { label: 'Estimation générale — à affiner en dégustant', emoji: '🧭' },
    }[source],
