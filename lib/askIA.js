...
// se rabat sur Claude/Gemini qui ont leur propre recherche web intégrée.
//
...
// Le palier gratuit Groq rejette (413) les requêtes trop volumineuses.
// Nos prompts système (base de vins entière, contexte Tavily…) peuvent
// largement dépasser cette limite — dans ce cas on saute Groq directement
// plutôt que de perdre du temps sur un aller-retour voué à l'échec.
const MAX_GROQ_CHARS = 16000
...
export async function askIA(payload) {
  ...
  if (needsWebSearch) {
    ...
    if (context) {
      ...
      groqPayload = null // pas de grounding trouvé → on saute Groq, direction Claude/Gemini natifs
    }
  }
  ...
  // 2. Repli Claude (gère aussi web_search nativement).
  try {
