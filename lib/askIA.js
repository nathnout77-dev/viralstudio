// Helper unique d'appel IA avec repli automatique Claude → Gemini.
//
...
// échoue ou renvoie une erreur, on retente /api/gemini avec le MÊME payload.
// Gemini renvoie une structure identique ({content:[...]}), donc l'appelant
// parse la réponse de la même manière quel que soit le fournisseur.
//
// Renvoie toujours { ok, data } pour refléter le pattern fetch d'origine :
// - ok:true  → data contient un content exploitable
// - ok:false → les deux fournisseurs ont échoué ; l'appelant affiche son
...
export async function askIA(payload) {
  ...
  try {
    ...
    return { ok: false, data: { error: 'both_failed' } }
  }
}
