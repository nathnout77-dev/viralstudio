...
function tooLargeForGroq(payload) {
  // Les images sont mesurées à part : leur base64 pèse des centaines de
  // milliers de caractères mais ne coûte que ~1500-2500 tokens aux modèles
  // vision — on les exclut donc du comptage de caractères.
  try {
    const sansImages = {
      ...payload,
      messages: (payload?.messages || []).map(m => Array.isArray(m.content)
        ? { ...m, content: m.content.filter(b => b?.type !== 'image') }
        : m),
    }
    return JSON.stringify(sansImages).length > MAX_GROQ_CHARS
  } catch {
    return true
  }
}
...
export async function askIA(payload) {
  // `webQuery` : requête de recherche courte fournie par l'appelant (ex. le
  // scanner : « Château X Anjou 2022 vin »). Sans elle, on retombe sur le
  // texte du dernier message utilisateur — correct pour le chat, mais
  // catastrophique quand ce message est un prompt d'instruction complet :
  // Tavily ne trouve rien avec 1800 caractères de règles JSON en guise de
  // requête. On la retire du payload avant tout appel fournisseur.
  const { webQuery, ...request } = payload || {}
  const needsWebSearch = wantsWebSearch(request?.tools)
  let groqPayload = request
  let sawRateLimit = false

  if (needsWebSearch) {
    // Tavily limite la requête à ~400 caractères — au-delà, erreur assurée.
    const query = String(webQuery || extractQuery(request?.messages)).slice(0, 380)
    const tavily = query ? await tavilySearch(query) : null
    const context = tavily ? formatTavilyContext(tavily) : ''
    if (context) {
      groqPayload = { ...request, system: `${request.system || ''}${context}` }
    } else {
  ...
  try {
    const gemini = await callProvider('/api/gemini', request)
  ...
  try {
    const claude = await callProvider('/api/claude', request)
