...
import { previenirParPush } from './push'
...
export async function envoyerMessage(amiId, contenu, vin = null) {
  ...
  const r = await sur(() => supabase.from('messages').insert({
  ...
  // Le message est en base : c'est l'essentiel, et c'est fait. Reste à
  // réveiller l'ami même s'il n'a pas Œno ouvert — sans attendre la réponse,
  // pour ne pas retarder l'affichage de la bulle chez l'expéditeur.
  if (r.ok) previenirParPush(amiId, texte, vin)
  return r
}
