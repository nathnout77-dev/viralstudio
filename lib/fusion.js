...
/**
 * Un snapshot réduit à ce qui appartient vraiment à l'utilisateur.
 *
 * `snapshotLocal` écarte déjà les bouteilles de démonstration à la source.
 * On recommence ici volontairement : cette fonction est le dernier verrou
 * avant qu'une cave soit écrasée, et elle ne doit pas dépendre de la
 * discipline de ses appelants. Le jour où un nouveau chemin lui présentera
 * une cave brute, la protection tiendra quand même.
 */
function possede(snap) {
  return snap ? { ...snap, cave: sansDemo(snap.cave) } : snap
}
 ...
 */
export function decisionFusion(brutLocal, brutCloud) {
  const local = possede(brutLocal), cloud = possede(brutCloud)
