...
export function computeSuggestionDuJour(wines = []) {
  ...
  if (fenetre.length > 0) {
    ...
    return {
      type: 'cave',
      // Même exigence qu'ailleurs : ne pas présenter une bouteille d'exemple
      // comme appartenant au visiteur. Il n'a encore rien mis dans sa cave.
      eyebrow: estVinDemo(w) ? 'Dans la cave d’exemple' : 'Dans votre cave',
