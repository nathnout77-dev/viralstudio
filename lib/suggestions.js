...
const PLAT_LABELS = {
  viande_rouge: 'votre viande rouge', viande_blanche: 'votre viande blanche',
  poisson: 'le poisson', pates: 'les pâtes', fromage: 'le fromage',
  vege: 'un repas léger', grillades: 'vos grillades', apero: "l'apéro",
}

// Viande précise (question conditionnelle du Sommelier express) : prioritaire
// sur le libellé générique du plat dans la phrase de raison.
const VIANDE_LABELS = {
  boeuf: 'votre bœuf', agneau: "l'agneau", gibier: 'le gibier',
  canard: 'le canard', poulet: 'la volaille', veau: 'le veau',
  porc: 'le porc', lapin: 'le lapin',
}
...
export function buildRaison(wine, criteres = {}) {
  ...
  // Plat (la viande précise, si connue, l'emporte sur le libellé générique)
  const platLabel = (c.viande && VIANDE_LABELS[c.viande]) || (c.plat && PLAT_LABELS[c.plat])
  if (platLabel) {
    extras.push(pick([
      `il accompagnera à merveille ${platLabel}`,
      `un accord tout trouvé avec ${platLabel}`,
    ], seed + 1))
  }
